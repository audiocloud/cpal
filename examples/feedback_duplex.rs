...
fn main() -> Result<(), anyhow::Error> {
    ...
    let io_stream = input_device.build_full_duplex_stream(
        &config,
        &config,
        |data_in: &[i16], data_out: &mut [i16], in_info, out_info| {
            // do nothing
        },
        err_fn,
    )?;

    println!("Successfully built full duplex streams.");
